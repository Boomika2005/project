...
def render_pdf_bytes(facts: dict, overlay_path: Path | None) -> bytes:
    ...
    # Results
    c.setFont("Helvetica-Bold", 12)
    c.drawString(m, y, "Results")
    y -= 8 * mm

    c.setFont("Helvetica", 10)
    c.drawString(m, y, f"Tumor Type: {facts.get('tumor_type', 'NA')}")
    y -= 5 * mm
    c.drawString(m, y, f"Tumor Size: {facts.get('tumor_size', 'NA')} px")
    y -= 5 * mm
    plane_info = facts.get("plane", {})
    predicted_plane = plane_info.get("predicted_label", "NA")
    c.drawString(m, y, f"MRI Plane: {predicted_plane}")
    y -= 10 * mm
    ...
    c.drawString(
        m,
        y,
        f"Patient ID: {patient.get('patient_id','NA')}    "
        f"Name: {patient.get('name','NA')}    "
        f"Age: {patient.get('age','NA')}    "
        f"Sex: {patient.get('sex','NA')}"
    )
    y -= 6 * mm
    ...
    c.drawString(
        m,
        y,
        f"Modality: {study.get('modality','NA')}    "
        f"Date: {study.get('study_date','NA')}"
    )
    y -= 10 * mm
